...
import { IssueStatus } from '../issues/entities/parking-lot-issue.entity';
import * as path from 'path';
import * as fs from 'fs/promises';

@Injectable()
export class ParkingLotMaintenanceService {
  constructor(
    @InjectRepository(ParkingLotMaintenance)
    private maintenanceRepository: Repository<ParkingLotMaintenance>,
  ...
  async create(createDto: CreateParkingLotMaintenanceDto) {
    ...
    await this.parkingLotsService.findOne(createDto.parkingLotCode);

    const existingActive = await this.maintenanceRepository.findOne({
      where: {
        parkingLotCode: createDto.parkingLotCode,
        status: In([ScheduleStatus.REQUESTED, ScheduleStatus.STARTED, ScheduleStatus.PAUSED]),
      },
    });
    if (existingActive) {
      throw new BadRequestException('There is already an active/requested maintenance schedule for this parking lot');
    }
    ...
    if (createDto.issueId) {
      ...
      await this.parkingLotIssuesService.updateStatus(createDto.issueId, {
  ...
  async findAll() {
    return this.maintenanceRepository.find({
      relations: ['parkingLot', 'performedBy', 'attachments'],
  ...
  async findOne(id: string) {
    ...
    if (!maintenance) {
      throw new NotFoundException(`Parking lot maintenance with ID ${id} not found`);
    }
    return maintenance;
  }

  async update(id: string, updateDto: UpdateParkingLotMaintenanceDto) {
    ...
    if (saved.status === ScheduleStatus.COMPLETED && previousStatus !== ScheduleStatus.COMPLETED) {
      if (saved.issueId) {
        await this.parkingLotIssuesService.updateStatus(saved.issueId, {
  ...
  async addAttachment(id: string, file: Express.Multer.File) {
    ...
    const attachment = this.attachmentsRepository.create({
      parkingLotMaintenanceId: maintenance.id,
