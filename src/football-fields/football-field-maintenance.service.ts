...
import { IssueStatus } from '../issues/entities/football-field-issue.entity';
import * as path from 'path';
import * as fs from 'fs/promises';

@Injectable()
export class FootballFieldMaintenanceService {
  constructor(
    @InjectRepository(FootballFieldMaintenance)
    private maintenanceRepository: Repository<FootballFieldMaintenance>,
  ...
  async create(createDto: CreateFootballFieldMaintenanceDto) {
    ...
    await this.footballFieldsService.findOne(createDto.footballFieldCode);

    const existingActive = await this.maintenanceRepository.findOne({
      where: {
        footballFieldCode: createDto.footballFieldCode,
        status: In([ScheduleStatus.REQUESTED, ScheduleStatus.STARTED, ScheduleStatus.PAUSED]),
      },
    });
    if (existingActive) {
      throw new BadRequestException('There is already an active/requested maintenance schedule for this football field');
    }
    ...
    if (createDto.issueId) {
      ...
      await this.footballFieldIssuesService.updateStatus(createDto.issueId, {
  ...
  async findAll() {
    return this.maintenanceRepository.find({
      relations: ['footballField', 'performedBy', 'attachments'],
  ...
  async findOne(id: string) {
    ...
    if (!maintenance) {
      throw new NotFoundException(`Football field maintenance with ID ${id} not found`);
    }
    return maintenance;
  }

  async update(id: string, updateDto: UpdateFootballFieldMaintenanceDto) {
    ...
    if (saved.status === ScheduleStatus.COMPLETED && previousStatus !== ScheduleStatus.COMPLETED) {
      if (saved.issueId) {
        await this.footballFieldIssuesService.updateStatus(saved.issueId, {
  ...
  async addAttachment(id: string, file: Express.Multer.File) {
    ...
    const attachment = this.attachmentsRepository.create({
      footballFieldMaintenanceId: maintenance.id,
